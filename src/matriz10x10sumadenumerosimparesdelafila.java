 ...
 * @author Andertaker
 */
public class matriz10x10sumadenumerosimparesdelafila {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<matriz.length;i++){
            for(int j=0;j<matriz.length;j++){
                if(i/2==0){
                sumafilapar[i] += matriz[i][j];
                }
            }
        }
        for(int i=0;i<matriz.length;i++){
            if(i/2==0){
        System.out.println("La suma de la filas pares: "+i+"es: "+sumafilapar[i]);
            }
        }  
    }
}
